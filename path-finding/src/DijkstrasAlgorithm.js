...
async function search(canCrossDiagonals) {
  ...
  currentCell.dijkstraShortest = 0;
  ...
  while (unvisitedHeap.lastHeapCellIndex >= 0) {
    ...
    unVisitedNeighbours.forEach((neighbour) => {
      ...
      if (neighbour.cellType !== CELL_TYPES.OBSTACLE) {
        ...
        const newDistanceFromStartToNeighbour =
          tempCurrentCell.dijkstraShortest +
        ...
        if (neighbour.dijkstraShortest > newDistanceFromStartToNeighbour) {
          // assign the new shortest distance
          neighbour.dijkstraShortest = newDistanceFromStartToNeighbour;
    ...
    if (currentCell.dijkstraShortest === Number.MAX_VALUE) {
      break;
    }
  }
  ...
  return new Promise((resolve) =>
    resolve(
      gridCl.grid.forEach((row) =>
        row.forEach((cell) => {
          cell.dijkstraShortest = Number.MAX_VALUE;
          heap.add(cell);
        })
      )
    )
